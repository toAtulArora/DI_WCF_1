...
Let 
\[
Q_{i}:=\mathbb{E}(X_{\Pi(1)}+\dots X_{\Pi(n-1)}|F^{i-1})
\]
where $\{F^{i}\}_{i=0}^{n}$ is a filteration which specifies $\Pi(i)$
and $F^{0}$ specifies $X_{1},X_{2}\dots X_{n}$. It is known that
the sequence $Q_{i}$ forms a martingale (see TODO: put a claim and
a simple proof). Further, $\left|Q_{i}-Q_{i-1}\right|\le d$ for some
$d>0$ (TODO: maybe show $d=2$ or something). We can therefore apply
the Azum-Hoeffding inequality (see TODO) to get 
\[
\Pr\left\{ \left|Q_{1}-Q_{n}\right|>r\right\} \le e^{-cr^{2}}
\]
for some constant $c>0$. Noting that 
\[
Q_{1}=\frac{1}{\left|S_{n}\right|}\sum_{\pi\in S_{n}}(X_{\pi(1)}+\dots X_{\pi(n-1)})
\]
and 
\[
Q_{n}=X_{\Pi(1)}+\dots X_{\Pi(n-1)}
\]
...
Let 
\[
Y_{i}=\frac{1}{\left|S_{n}\right|}\sum_{\pi\in S_{n}}X_{\pi(i)},\quad\text{for }i\in\{1,2\dots n\}.
\]
Note that $Y_{1}=Y_{2}\dots=Y_{n}$. Define 
\[
Y:=\frac{1}{n}\sum_{i=1}^{n}Y_{i}.
\]
We want to estimate 
\begin{align*}
\frac{1}{\left|S_{n}\right|}\sum_{\pi\in S_{n}}\mathbb{E}(X_{\pi(n)}) & =\mathbb{E}\left(\sum_{\pi\in S_{n}}\frac{X_{\pi(n)}}{\left|S_{n}\right|}\right)\\
 & =\mathbb{E}(Y).
\end{align*}
This motivates the following definition 
\[
Z_{i}:=\mathbb{E}(Y|F^{i-1})
\]
where the filteration $F^{i-1}$ fixes $\Pi=\pi$ and $X_{\pi(1)},X_{\pi(2)}\dots X_{\pi(i-1)}$,
and $F^{0}$ is the trivial filteration. Now, $\{Z_{i},F^{i}\}$ form
a martingale (TODO: put that examlpe), and $\left|Z_{i+1}-Z_{i}\right|<d$
for some fixed $d$. From Azum-Hoeffding we have that 
\[
\Pr\left\{ \left|Z_{0}-Z_{n}\right|>r\right\} \le2e^{-cr^{2}}
\]
for some $c>0$. Noting that 
\begin{align*}
Z_{n} & =Y\\
 & =\frac{\left(\sum_{\pi\in S_{n}}X_{\pi(1)}+X_{\pi(2)}+\dots X_{\pi(n-1)}\right)+\sum_{\pi\in S_{n}}X_{\pi(n)}}{n\left|S_{n}\right|}\\
 & =\frac{n}{n-1}\frac{\left(\sum_{\pi\in S_{n}}X_{\pi(1)}+\dots X_{\pi(n-1)}\right)}{n.\left|S_{n}\right|}
\end{align*}
and 
\begin{align*}
Z_{0} & =\mathbb{E}(Y)\\
 & =\frac{1}{\left|S_{n}\right|}\sum_{\pi\in S_{n}}\mathbb{E}(X_{\pi(1)}+\dots X_{\pi(n)})
\end{align*}
we obtain the result.
\end{proof}
}
