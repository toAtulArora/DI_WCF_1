...
\item As in the previous proof, either $x\oplus g=1$, in which case Bob
is convinced he won or $x\oplus g=0$ in which case he will test Alice.
We proceed under the latter assumption, i.e. suppose that $x=g$.
The other case will be captured by normalisation (explained shortly).
\item Alice sending $a$ can be modelled as introducing $\rho_{2}\in ASIJG$
satisfying $\tr_{A}(\rho_{2})=\rho_{1}\otimes\frac{\mathbb{I}_{G}}{2}$.

To model the GHZ test, introduce a register $Y$ in the state $\frac{\left|0\right\rangle _{Y}+\left|1\right\rangle _{Y}}{\sqrt{2}}$.
Recall that since we assumed $x=g$ and that to perform the GHZ test,
we need $x\oplus y\oplus z=1$ i.e. $z=1\oplus y\oplus g$. Further
introduce registers $B$ and $C$ to hold the measurement results
and with the aforesaid in mind, define 
\begin{equation}
U:=\sum_{y,g\in\{0,1\}}\left|y\right\rangle \left\langle y\right|_{Y}\left|g\right\rangle \left\langle g\right|_{G}\otimes(\mathbb{I}_{B}\otimes\Pi_{0|y}^{I}+X_{B}\otimes\Pi_{1|y}^{I})\otimes(\mathbb{I}_{C}\otimes\Pi_{0|(1\oplus y\oplus g)}^{J}+X_{C}\otimes\Pi_{1|(1\oplus y\oplus g)}^{J})\otimes\mathbb{I}_{AS}.\label{eq:Bob-u}
\end{equation}
By construction, $\rho_{3}:=U\left(\left|+\right\rangle \left\langle +\right|_{Y}\otimes\left|00\right\rangle \left\langle 00\right|_{BC}\otimes\rho_{2}\right)U^{\dagger}\in YBCASIJG$
...
Define 
\[
\Pi^{{\rm sTest}}:=\sum_{s,a,x:s=a\lor s=a\oplus x}\left|sax\right\rangle \left\langle sax\right|_{SAG}\otimes\mathbb{I}_{YBCIJ}
\]
 to model the first test, i.e. $s$ should either be $a$ or $a\oplus x$
(and used the assumption $x=g$). Define 
\[
\Pi^{{\rm GHZ}}:=\sum_{\substack{x,y\in\{0,1\},\\
a,b,c\in\{0,1\}:a\oplus b\oplus c\oplus1=xy\cdot(1\oplus x\oplus y)
}
}\left|xyabc\right\rangle \left\langle xyabc\right|_{GYABC}\otimes\mathbb{I}_{SIJ}
\]
to model the GHZ test. Let 
\begin{equation}
\Pi_{0}:=\Pi^{{\rm GHZ}}\Pi^{{\rm sTest}},\quad\Pi_{\perp}:=(\mathbb{I}-\Pi^{{\rm sTest}})(\mathbb{I}-\Pi^{{\rm GHZ}}),\quad\text{and}\quad\Pi_{1}:=\mathbb{I}-\Pi_{0}-\Pi_{\perp}.\label{eq:BobProjs}
\end{equation}
...
\[
(\alpha,\beta,\gamma)=(\tr(\Pi_{0}\rho_{3}),\tr(\Pi_{1}\rho_{3}),\tr(\Pi_{\perp}\rho_{3})).
\]
\end{enumerate}
To summarise, the final SDP is as follows: let $\rho_{0}\in IJ$ be
as defined in \Eqref{Bob_initState}, $\rho_{1}\in SIJ$ and $\rho_{2}\in ASIJG$.
Then, 
\[
\max\quad\tr\left([c_{0}\Pi_{0}+c_{1}\Pi_{1}+c_{\perp}\Pi_{\perp})U\left(\left|+00\right\rangle \left\langle +00\right|_{YBC}\otimes\rho_{2}\right)U^{\dagger}\right)
\]
subject to 
\begin{align*}
\tr_{S}(\rho_{1}) & =\rho_{0}\\
