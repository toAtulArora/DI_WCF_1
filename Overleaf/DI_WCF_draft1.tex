...
\begin{assumption}[Abort means I won]
If an honest player aborts, they consider themselves the winner,
i.e. an honest player either outputs $A$ or $B$ but not $\perp$.\label{assu:noAbort}
\end{assumption}

\branchcolor{purple}{TODO: maybe explain this a little better? }
...
be their polarisations (see \Defref{unbalanced-polarity}). All protocols
are executed under \Assuref{noAbort}. Define $C_{{\rm stand}}(\mathcal{X},\mathcal{Y})$
as follows:
\begin{enumerate}
...
(see \Algref{AliceSelfTestsHerBoxes}) and recall (see \Lemref{AliceSelfTests})
\begin{align*}
...
\begin{rem}
Again, even though it seems that Alice might be favoured, $C_{{\rm AAC}}$
may result in a protocol with the opposite polarity---let $\mathcal{Z}:=C_{{\rm AAC}}(\mathcal{X},\mathcal{Y})$,
then $\mathcal{Z}_{A}$ is not necessarily the same as $\mathcal{Z}$. 
\end{rem}

\branchcolor{purple}{As before, to be concrete, let us workout an example. Consider protocol
$Q$ (see \Algref{BobSelfTests}) and let $(v_{A},v_{B},v_{\perp})\in\mathbb{C}_{A}(\mathcal{Q}_{A})$
be a cheat vector for $\mathcal{Q}$ (in this discussion, we reserve
$\alpha,\beta$ for notational simplicity). Further, recall that 
\begin{align*}
...
$p_{B}^{*}(\mathcal{Q}_{B})\le\alpha$. Let $\mathcal{Q}':=C_{{\rm AAC}}(\mathcal{Q},\mathcal{Q})$,
...
(see \Figref{Abort-Augmented-Composition}). Our goal is to obtain
an upper bound on $p_{A}^{*}(\mathcal{Q}')$ and $p_{B}^{*}(\mathcal{Q}')$.
We start from the bottom of \Figref{Abort-Augmented-Composition}. 
...
bounds. The second inequality holds because $\alpha>\beta$ so putting
all the maximum weight on $\alpha$ (i.e. $\alpha$) while the remaining
on $\beta$ (i.e. $1-\alpha$) is the best one can do. Operationally,
...
can consider $\mathcal{Q}'':=\mathbb{C}_{{\rm AAC}}(\mathcal{Q},\mathcal{Q}')$
...
\par\end{centering}
\caption{Abort Augmented Composition technique. (TODO: improve the caption)\protect \\
...
Let $X\in\{A,B\}$. Then 
\[
p_{X}^{*}(\mathbb{C}_{{\rm AAC}}(\mathcal{Q}))\apprle0.8175\dots
\]
and 
\[
p_{X}^{*}(\mathbb{C}_{{\rm AAC}}(\mathcal{Q}))\apprle
\]
(TODO: calculate). 
\end{thm}
...
Let $X\in\{A,B\}$, 
\begin{align*}
\mathcal{Z}^{1} & :=\mathbb{C}_{{\rm AAC}}(\mathcal{Q},\mathcal{P}),\quad{\rm and}\\
\mathcal{Z}^{i+1} & :=\mathbb{C}_{{\rm AAC}}(\mathcal{Q},\mathcal{Z}^{i})\quad i>1.
\end{align*}
Then 
\[
...
\branchcolor{purple}{TODO: Argue why $\mathbb{C}_{{\rm AAC}}(\mathcal{P})$ was not such
a great idea? Perhaps do the simulation and add the results. }
...
\begin{thm}
\label{thm:Martingale}Let $\{X_{1},X_{2}\dots X_{n}\}$ be $n$ random
variables which are possibly correlated and take values in $\{0,1\}$.
Let $\Pi\in S_{n}$ be a random variable (which takes values in the
permutation group), picked from the uniformly random distribution.
Then, 
\[
\Pr\left\{ \left|\frac{X_{\Pi(1)}+\dots X_{\Pi(n-1)}}{n-1}-\mathbb{E}(X_{\Pi(n)})\right|>r\right\} \le\exp(-cnr^{2})
\]
for some fixed $c>0$. 
\end{thm}

\begin{lem}
Let $X_{i}$s and $\Pi$ be as in \Thmref{Martingale}, above. Then
\[
\Pr\left\{ \left|X_{\Pi(1)}+\dots+X_{\Pi(n-1)}-\frac{1}{\left|S_{n}\right|}\sum_{\pi\in S_{n}}\left(X_{\pi(1)}+\dots X_{\pi(n-1)}\right)\right|\ge r\right\} \le2e^{-\frac{cr^{2}}{n}}
\]
for some $c>0$. 
\end{lem}

\begin{lem}
Let $X_{i}$s be as in \Thmref{Martingale}, above. Then 
\[
\Pr\left\{ \left|\frac{1}{\left|S_{n}\right|}\sum_{\pi\in S_{n}}\left(\frac{n}{n-1}(X_{\pi(1)}+\dots+X_{\pi(n-1)})-\mathbb{E}(X_{\pi(1)}+\dots+X_{\pi(n)}\right)\ge r\right|\right\} \le2e^{-\frac{cr^{2}}{n}}
\]
for some $c>0$. 
\end{lem}
