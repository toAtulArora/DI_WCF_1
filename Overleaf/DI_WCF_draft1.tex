...
\affil[4]{Institute of Quantum Computing}
...
\maketitle
\begin{abstract}
Prior to this work, the best device independent weak coin flipping protocol had bias, $\epsilon \le 0.33664$, introduced a decade ago {[}10.1103/PhysRevLett.106.220501{]}. We report a protocol with bias, $\epsilon \approx 0.3148$. Under a plausible continuity conjecture, we are able to lower the bias to $\epsilon \approx 0.29104$. Our result uses the aforementioned protocol with a minor modification and owes its improved security to two techniques which we expect should work more generally: an added self-testing step and an improved method for composing protocols.
...
Indeed, Silman, Chailloux, Aharon, Kerenidis, Pironio and Massar \sout{\Jnote{author names}} presented a protocol in~\cite{Silman2011} which has bias $\eps \approx 0.33664$. 
...
\begin{lyxalgorithm}[DI-WCF protocol with $\PA = \cos^2{\pi/8}$ and $\PB = 3/4$~\cite{Silman2011}; see \Subsecref{SCForiginal}] \label{alg:SCForiginalJ}  
...
To obtain a bias $\eps \le 0.33664 $ protocol from the above, they compose the protocol many times \Jnote{Did they discuss how?} \Anote{A little} \Anote{Perhaps also mention that their protocol was actually a strong coin flipping protocol?}. 
...
In this work, we build on this protocol using novel pre- and post-processing steps, which we discuss in the next subsection. 
	...
	\begin{thm} 
	There exists device-independent weak coin flipping protocols with bias approaching $\eps \approx 0.3148$. Under a convergence assumption, the bias can be lowered to $\eps \approx 0.29104$. \Anote{I added this last line!}
	\end{thm} 
	...
	Otherwise, she announces to Bob that they can use the remaining boxes for Protocol~\ref{alg:SCForiginalJ}.   
	\end{enumerate} 
	\end{lyxalgorithm}  
	...
	\sout{Thus} Consequently, we study a family of protocols where the cheating probabilities approach certain thresholds. 
	...
	\subsection{Second improvement: abort-phobic composition} 
	\Tnote{changed title of the subsection, the paragraph name and restructured the first two paragraphs.}

    It can happen, that for a given WCF protocol, $\PB \neq \PA$, in which case we say the protocol is polarised. It is known (e.g. \cite{Silman2011}) \sout{from the works of \Tnote{Add reference}} that compositing the protocol with itself or other protocols can then effectively reduces the bias $\eps$. Our second improvement is a better way of compositing protocols, when there is a probability that the honest player catches the cheating player. Let us start by recalling the standard way of composing protocols.

	\paragraph{Standard composition.} 
	For a protocol with cheating probabilities, we say that it has polarity towards Alice when it satisfies $\PA > \PB$. Similarly, we say that it has polarity towards Bob when $\PB > \PA$. Given a polarized protocol $\mathcal{P}$, we may switch the roles of Alice and Bob since the definition of coin-flipping is symmetric. To make the polarity explicit, we define $\mathcal{P}_A$ to be the version of the protocol with $\PA > \PB$ and $\mathcal{P}_B$ to be the version with $\PB > \PA$. 
	...
	\item If Alice won, she polarizes the second protocol towards herself, i.e., they now use the protocol $\mathcal{P}_A$ to determine the outcome of the (entire) protocol. 
	\item If Bob won, he polarizes the second protocol towards himself, i.e., they now use the protocol $\mathcal{P}_B$ to determine the outcome of the (entire) protocol.   
	\end{enumerate} 
	\end{lyxalgorithm}  
	...
	In other word, the Equations~\ref{first} and \ref{second} may \sout{be able to} get reduced even further. 
	...
	\begin{tabularx}{\textwidth}{lX}
		...
		$\gamma$ is the probability with which Bob aborts. \\ 
	\end{tabularx} 
	\end{center}  
	Cheat vectors for (dishonest) Bob are analogously defined.
	\end{defn} 
	...
	For instance, suppose Alice and Bob are now using a protocol midway through the protocol. \Anote{Umm, the word protocol appears twice; maybe rephrase?}
	...
	\end{array} \right\}  
	\end{equation} 
	\Tnote{Shouldn't this be "Alice wins the second protocol, when biased toward her"} where she may choose any strategy with cheat vector $(\alpha, \beta, \gamma)$ that she wishes for the current protocol. 
	...
	By using self-testing and abort-phobic compositions, we are able to find protocols which converge onto a bias of $\eps \approx 0.3148$ proving the main result of this work. By composing protocols where Alice self-tests with ones where Bob self-tests, under a convergence assumption, we are able to reduce the bias further down to $\eps \approx 0.29104$, as also stated in the main result. \Anote{Added this last line.}
...
% \section{Introduction}
	...
	\subsection{Results: application of improvements to Weak Coin Flipping \label{subsec:Proof-Technique}}
		...
		Let $\mathcal{I}$ denote \Algref{SCForiginalJ} (the DI protocol introduced in \cite{Silman2011}). We use $p^*_A(\mathcal{I})$ and $p^*_B(\mathcal{I})$, for instance, to denote the cheating probabilities of Alice and Bob when they execute protocol $\mathcal{I}$. Similarly, we use $\mathbb{C}_A(\mathcal{I})$ and $\mathbb{C}_B(\mathcal{I})$, for instance, to denote the cheat vectors for Alice and Bob, respectively, when the execute $\mathcal{I}$.
		%}
    ...
    \Tnote{WHY LL notation ?}
		%\branchcolor{purple}{
		We introduce two variants of protocol $\mathcal{I}$ (due to \cite{Silman2011}), which we call
		...
		an additional test to ensure Bob doesn't cheat. We show that $p_{A}^{*}(\mathcal{P})\approx 0.853\dots$
		and $p_{B}^{*}(\mathcal{P}) \approx 0.667\dots$. We also show that the set of cheat vectors
		...
		\[
		\epsilon(C^{LL}(\mathcal{I}))\approx 0.33664\dots
		\]
		while for our improved protocol $\mathcal{P}$, it is given by
		\begin{equation}
		\epsilon(C^{LL}(\mathcal{P}))\approx 0.3199\dots.\label{eq:SikoraP}
		\end{equation}
		\end{itemize}
		...
		technique that uses ``cheat vectors'' and the abort event as explained. We denote the new protocol
		obtained using this improved ``abort phobic'' composition (of protocol
		$\mathcal{X}$) by either $C_{\perp L}(\mathcal{X})$ or $C_{L\perp}(\mathcal{X})$. The notation is explained in later sections.
		\begin{itemize}
		\item Composing the protocol $\mathcal P$ with itself $n$ times, using the improved compositions gives a bias
		\[
		\epsilon(C^{\perp L}(\mathcal{P}))\approx 0.3148\dots
		\]
		which is a further improvement.
		\item Composing the protocol $\mathcal Q$ with itself $n$ times, using the improved composition gives a bias 
		\[
		\epsilon(C^{L\perp}(\mathcal{Q}))\approx 0.3226\dots
		\]
		for all $X\in\{A,B\}$, which is worse than even \Eqref{SikoraP}. \Tnote{Where is X ?}
		\item However, when we compose the protocol $Q$ $n$ times with itself followed by $P$%
		\footnote{%
		    $C^{\perp L}(\mathcal{P},\mathcal{P},\dots,\mathcal{P},\mathcal{Q})$
		is strictly worse than considering $C^{\perp L}(\mathcal{P},\mathcal{P},\dots,\mathcal{P},\mathcal{P})$;
		this should become evident later.%
		}%
		, we find (again, for
		all $X\in\{A,B\}$)
		\[
...
%\branchcolor{purple}{
We describe device independent protocols as classical protocols with
...
%\branchcolor{purple}{
Before we define the current best DI CF protocol, we briefly remind
...
which is the same as before, but $P_{B}^{*} \approx 0.667...$.}
...
\[
\mathbb{C}_{B}(\mathcal{I}):=\{(\alpha,\beta,\gamma):\exists\text{ a strategy of \ensuremath{B} s.t. an honest \ensuremath{A} outputs }\text{0, 1, and \ensuremath{\perp} with probabilities \ensuremath{\alpha,\beta} and \ensuremath{\gamma}}\}
\]
