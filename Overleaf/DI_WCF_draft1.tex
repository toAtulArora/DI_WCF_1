...
\newcommand{\aspace}{\hspace{\algorithmicindent}}
...
\begin{algorithmic}[1]
	...
	\STATE For $i \in [n]\backslash J$, play the GHZ game with box $i$ with result $X_i\in \{0,1\}$ 
	\STATE Verify if 
	\begin{IEEEeqnarray}{r'L}
		\Omega:&\frac{1}{n-1}\sum_{i\in [n] \backslash J} X_i \geq\mu
	\end{IEEEeqnarray}
	\STATE If the test $\Omega$ passes, conclude that 
	\begin{IEEEeqnarray}{r'L}
		T:& E[X_J|J] \geq \mu - \delta\,,
	\end{IEEEeqnarray}
\end{algorithmic}
\end{algorithm}
...
\begin{prop}[Security statement]
	\label{prop:security}
	For any implementation of the boxes, the joint probability that that the test $\Omega$ passes and that the conclusion $T$ is false is exponentially small: $\Pr[ \Omega \cap \bar T] \leq \epsilon = e^{-c\delta^2(n-1)}$ for some constant $c$.
\end{prop}

Note that in practice we can chose the free parameters $\mu$ and $\delta$ such that $\mu \rightarrow 1$, $\delta \rightarrow 0$ and $\epsilon \rightarrow 0$ when $n\rightarrow \infty$, so in the asymptotic limit we can certify that $\mathbb{E}[X_J|J] = 1$ with arbitrary confidence. 

\begin{lem}[Azuma-Hoeffding]
	Let $0\leq X_i \leq 1$ be iid. bounded random variables, with $E[X_i] = \mu$, and $\delta >0$ then
	\begin{IEEEeqnarray}{rL}
		\Pr\left[ \left|\frac{1}{n} \sum_{i= 1}^n X_i - \mu\right| \geq \delta\right] \leq e^{-cn\delta^2}\,,
	\end{IEEEeqnarray}
	for some constant $c$.
\end{lem}

\begin{proof}[Proof of Proposition~\ref{prop:security}]
	Denote by $\vect p = p(x_1,\cdots x_n)$ the joint distribution of the results of the GHZ games, for a given strategy (states and measurements) implemented by the adversary. 
	
	We first show that, when proving the security, we can consider without loss of generality a symmetric distribution $\vect p$ for the GHZ tests. For this, we note that the quantity $\Pr[\Omega \cap \bar T]_{\vect p}$ is invariant under permutations $\vect p \mapsto \sigma\cdot \vect p$, with $\sigma \in S_n$, which permuting the order of the variables $x_i$. Since the probability distribution $\Pr[\Omega \cap \overline T]_{\vect p}$ is also linear in $\vect p$, we find $\Pr[\Omega \cap \overline T]_{\vect p} = \frac{1}{|S_n|} \sum_{\sigma \in S_n} \Pr[\Omega \cap \bar T]_{\sigma\cdot \vect p} = \Pr[\Omega \cap \bar T]_{\bar {\vect p}}$, where $\bar{\vect p} = \frac{1}{|S_n|}\sum_{\sigma \in S_n} \sigma \cdot \vect p$ is the symmetrized version of $\vect p$. 
	
	Furthermore, we find that, under a symmetric attack $\bar{\vect p}$, we can assume without loss of generality that $J$ takes on the fixed value $J = 1$. Indeed, we have
	\begin{IEEEeqnarray}{rL}
		\Pr[\Omega \cap \overline T]_{\bar{\vect p}}
			&= \sum_{j\in [n]} \tfrac{1}{n} \Pr[\Omega \cap \overline T|J = j]_{\bar{\vect p}}\\
			&= \Pr[\Omega \cap \overline T|J=1]_{\bar{\vect p}}
	\end{IEEEeqnarray}
	where we have conditioned and averaged over the possible values taken by $J$ and then noted that each term is equal because $\bar{\vect p}$ is symmetric.
	
	We now only need to upper-bound $\Pr[\Omega\cap \overline T|J=1]_{\bar{\vect p}}$. Since $J = 1$ is fixed, the event $T$ is either true or false. When it is true then $\Pr[\Omega \cap \overline T]_{\bar{\vect p}} = 0$, so the result is trivial. Assuming it is false (so that $E[X_1]_{\bar{\vect p}} < \mu - \delta$), then the event $\Omega$ implies the event $\frac{1}{n-1}\sum_{i=2}^n X_i  -\mathbb{E}(X_1) \geq\delta$ and so we find
	\begin{IEEEeqnarray}{rL}
		 \Pr[\Omega\cap \bar T]_{\bar {\vect p}} \leq \Pr\left[\tfrac{1}{n-1}\sum_{i=2}^n X_i - E[X_1] \geq  \delta \right]_{\vect q} \leq e^{-c(n-1)\delta^2}
	\end{IEEEeqnarray}
	where we concluded using the Azuma-Hoeffding inequality and used the fact that the symmetrised distribution satisfies $E[X_i] = E[X_1]$ for all $i \in [n]$.
\end{proof}

Alternative statement
\begin{thm}
\label{thm:Martingale}
Let $\{X_{1},X_{2}\dots X_{n}\}$ be $n$ random
variables which joint distribution $p(x_1,\cdots x_n)$ and take values in $\{0,1\}$.
Let $N\in \{1,\cdots n\}$ be a uniform random variable.
Then, 
\[
    \Pr\left[ \left| \frac{1}{n-1}\sum _{i \in \{1,\cdots n\}\backslash N} X_i -\mathbb{E}(X_{N}|N)\right|>r\right]_{p} \le\exp(-cnr^{2})
    \label{eq:application_martingale}
\]
for some fixed $c>0$ and $\mathbb{E}(X_N|N) = \sum_{x_N = 0}^1  x_N.p(x_N)$
\end{thm}

\begin{proof}
    First note that the probability in the left hand side (LHS) of \eqref{eq:application_martingale} is linear in the distribution $p(x_1,\cdots x_n)$ (as any probability) and also invariant under permutations  $\sigma \in S_n$ of that distribution $p(x_1,\cdots x_n) \mapsto \simga\cdot p'(x_1,\cdots x_n) = p(x_{\sigma(1)}, \cdots x_{\sigma(n)})$, because $N$ was chosen uniformly at random. We therefore conclude that the RHS of \eqref{eq:application_martingale} takes on the same value if we evaluate if for the symmetrised distribution $\tilde p(x_1,\cdots x_n) = \sum_{\sigma \in S_n} p(x_{\sigma(1)}, \cdots x_{\sigma(n)})$. 
    
    Let $\{\tilde X_1,\cdots \tilde X_n\}$ be the symmetrized random variables with joint distribution $\tilde p(\tilde x_1,\cdots \tilde x_n)$. Then we note that for all $i\in\{1,\cdots n\}$, $\mathbb{E}(\tilde X_i) = \mathbb{E}(X_N) $. Furthermore  
    
    Under the symmetrised distribution, we have
    
    
    Define the random variable $Y_i = \sum_i X_i$
\end{proof}
...
\[
\mathbb{P}\left(\left|Z_{n}-Z_{0}\right|\ge0\right)\le2\exp\left(-\frac{r^{2}}{2\sum_{k=1}^{n}d_{k}^{2}}\right).
\]
\end{thm}
