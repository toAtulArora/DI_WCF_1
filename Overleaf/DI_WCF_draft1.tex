...
In this work, we build on this protocol using novel pre- and post-processing steps, which we discuss in the next subsection. 
	...
	\end{equation} 
	\end{lem}  
    \Tnote{remind the bias of the Inital protocol for easy comparison}
    ...
    It can happen, that for a given WCF protocol, $\PB \neq \PA$, in which case we say the protocol is polarised. It is known from the works of \Tnote{Add reference} that compositing the protocol with itself or other protocols can then effectively reduces the bias $\eps$. Our second improvement is a better way of compositing protocols, when there is a probability that the honest player catches the cheating player. Let us start by recalling the standard way of composing protocols.

	\paragraph{Standard composition} 
	For a protocol with cheating probabilities, we say that it has polarity towards Alice when it satisfies $\PA > \PB$. Similarly, we say that it has polarity towards Bob when $\PB > \PA$. Given a polarized protocol $P$, we may switch the roles of Alice and Bob since the definition of coin-flipping is symmetric. To make the polarity explicit, we define $P_A$ to be the version of the protocol with $\PA > \PB$ and $P_B$ to be the version with $\PB > \PA$. 
