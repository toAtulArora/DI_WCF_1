...
2. Protocols (First technique)
	[remark] This is where all the protocols appear
...
\date{October 5, 2021}                     %% if you don't need date to appear
...
%\Anote{Add a discussion about "adversarial noise" somewhere.}


% In this work, we \emph{revise} security definitions such that when Alice or Bob cheat, they have control of each other's quantum devices, opening up a plethora of new cheating strategies that were not considered in the previously mentioned references. 
...
%obtain bias $\eps \le 0.33664$. 
%has bias $\eps \le 0.33664$. To obtain a bias $\eps \le 0.33664$ using the protocol above, they compose the protocol many times. 

% In this work, we provide two techniques which can be applied to a wide range of protocols (including \Algref{SCF}, mentioned above) that can improve the bias. 
...
%To illustrate our ideas, we recall their protocol (see \Algref{SCF}).  
...
%In this work, we build on this protocol using two techniques, which we discuss next. 
	...
	\begin{thm} 
	There exist device-independent weak coin flipping protocols with bias, $\eps$, approaching $0.29104$. %Under a convergence assumption, the bias can be lowered to $\eps \approx 0.29104$.
	\end{thm} 
...
%\Tnote{Maybe comment on the fact the two techniques combined give an improvement (to clarify the fact that the first technique alone does not given an improvement} %Thanks. Indeed this part wasn't clear. The statement is not exactly true. I have explained it better later.
	...
	% It is relatively simple to observe that under the perfect self-testing assumption (i.e. the success probability is assumed to be unity), the optimisation problem corresponding to the cheating probability, can be cast as a semi-definite program. However, when the success probability is less than one (which will be the case in any realistic scenario), it is unclear if the optimization problem is over operators of bounded dimensions, let alone casting it as an SDP. This renders the ideal security guarantee hollow, until a continuity argument can be provided which asserts that the value of the realistic optimization problem approaches that of the ideal case, the aforementioned semi-definite program. These concerns are addressed in \Subsecref{SDPcontinuity}.
...
% \Anote{
% TODO: Fix these three paragraphs. Here's what I want to say.
% \begin{itemize}
	% \item Robust self-testing results are usually stated in terms of expected success probabilities in tests. Implicitly, multiple identical boxes are assumed to be available.\footnote{When this iid assumption is dropped, it is usually in the context of extracting randomness or key from the observed statistics, i.e. all the devices are used/measured. In our setting, we need to leave one device unused in the pre-processing step.} In our cryptographic setting, such an assumption is unwarranted. Hence, we estimate this expected success probability, by measuring $n-1$ boxes. This estimate requires, to the best of our knowledge, a novel analysis which we discuss in \Subsecref{EstimateGHZ}. 
	% \item It is relatively simple to observe that under the perfect self-testing assumption (i.e. the success probability is assumed to be unity), the optimisation problem corresponding to the cheating probability, can be cast as a semi-definite program. However, when the success probability is less than one (which will be the case in any realistic scenario), it is unclear if the optimization problem is over operators of bounded dimensions, let alone casting it as an SDP. This renders the ideal security guarantee hollow, until a continuity argument can be provided which asserts that the value of the realistic optimization problem approaches that of the ideal case, the aforementioned semi-definite program. These concerns are addressed in \Subsecref{SDPcontinuity}.
% \end{itemize}}
    ...
    It can happen, that for a given WCF protocol, $\PB \neq \PA$, in which case we say the protocol is \emph{polarised}. %\Jnote{American or British English?}. 
...
\Anote{If we're combining the full proofs, perhaps it is better to put the $\epsilon$ version here.}
...
Bob,\footnote{For Strong Coin Flipping, $P^*_B$ is max{prob(Bob can force Alice to output 1),prob(Bob can force Alice to output 0)}; $P^*_A$ is analogously defined.} $p_{B}^{*}(\mathcal{S})\le\frac{3}{4}$ and that of cheating
...
{
% \Anote{
% 	Things I want to convey in this section.
% 	\begin{itemize}
% 		\item Since tampering with the device doesn't help, we consider the "weak" variant.
% 		\item Describe Protocol W. Assert that the extra test doesn't improve the security but the optimal cheating strategy no longer uses GHZ. 
% 		\item Self-testing therefore might help. Clarify that both players can't simultaneously self test.
% 		\item Explain the advantage with Alice self-testing.
% 		\item Explain the advantage with Bob self-testing.
% 	\end{itemize}}
	
% While we already motivated our self-testing idea in the introduction, we discuss it here in more detail. 
...
% for the honest player. We omit the details here but mention it anyway to motivate the definition of \Algref{WCF}.
...
over $\mathbb{C}_{B}(\mathcal{Q})$ approaches that of an SDP. 
...
%\branchcolor{purple}{As already indicated in \Subsecref{Proof-Technique}, we do not expect the cheating probabilities to improve but we do expect an SDP characterisation of Alice's cheat vectors.}
...
that of an SDP. %semi definite programme. 
\end{lem}
...
\begin{figure}
	\label{fig:Standard-composition-technique}
\begin{centering}
\includegraphics[scale = 1]{figures/composition_1.pdf}
\par\end{centering}
\caption{Standard composition of weak coin flipping protocols. Subprotocols only have two outcomes depending on the coin flip. Labels indicate probabilities of outcomes for cheating Alice (blue) and cheating Bob (red)} 
\end{figure}
}
\begin{thm}
...
Then $p_{X}^{*}(C^{LL}(\mathcal{P}))\approx0.8199$ and $p_{X}^{*}(C^{LL}(\mathcal{X}))\approx0.836$. 
\end{thm}
...
abort, $\perp$ so we keep that output in the illustration, \Figref{fig:Abort-Augmented-Composition}.
...
were quite small and therefore one could hope that $\mathcal{Q}$ (which denotes \Algref{alg:BobSelfTests})
...
\begin{thm}
Let $\mathcal{P}$ and $\mathcal{Q}$ denote \Algref{AliceSelfTests} and \Algref{BobSelfTests} respectively. Suppose $X\in\{A,B\}$. Then 
\[
...
Then 
\[
\lim_{i\to\infty}p_{X}^{*}(\mathcal{Z}^{i})\approx0.791044.
\]
\end{thm}
...
\Anote{
	\begin{itemize}
		\item Say we begin with assuming that the self testing step guarantees, up to isometries, the state and measurements are $\epsilon$ close where $\epsilon$ is a function of $n$ and vanishes with large $n$.		
		\item 
	\end{itemize}
}
...
\label{subsec:SDP-when-Alice}

%\snote{Atul, in your arguments, you trace out Alice's junk. This may reveal info to Bob. Not sure. I think this is because the unitary acts on Alice's whole space, including the junk. But maybe I am mixed up.}

%\snote{Does the self-testing result imply Alice's measurement must act like that? It seem to imply something on all three boxes, but we only use it for Alice's. I think so, so perhaps this is a note to myself to think about it more :)}

%\begin{lem} 
%If $\| x - y \|_2 \leq \gamma$ then $\| xx^* - yy^* \|_2 \leq 2\gamma$. 
%\end{lem} 

%\begin{proof} 
%Easy. Will do it later if this is kept. 
%\end{proof} 

%\begin{lem} 
%For any choice of quantum states $\rho$ and $\sigma$, we have 
%\begin{equation} 
%\| \rho - \sigma \|_{tr} \leq \| \tr_B(\rho) - %\tr_B(\sigma) \|_{tr}. 
%\end{equation} 
%\end{lem} 

%\begin{proof} 
%This is in Mike and Ike. 
%\end{proof} 

%\snote{We might be able to combine both of the above using something in Mike and Ike.}

\begin{lem} 
	UPPER BOUND:  \snote{We need to multiply by some bound on the probability that Alice does not catch Bob cheating in the self-test stage. That probability needs to be incorporated somewhere. It shouldn't affect the results though.}
	\begin{align} 
	...
	\end{equation}
	 be an optimal solution to the $\eta_{^\epsilon}$  SDP (which exists by compactness) \snote{check!}. Then, again by compactness, the sequence $\{ (X^{1/t}) : t \in \mathbb{N} \}$ has an accumulation point (a limit of a convergent subsequence). Let $X' = (\rho'_1, \rho'_2, \rho'_3, y_{a,x}')$ be this accumulation point. 
	...
	Note that we have 
	\begin{equation} 
	\Pi_{a|x} \ket{GHZ} = \sqrt[4]{\text{prob}(a|x)} \ket{\psi'_{x,a}}_H \ket{\psi_{x,a}}_{IJ}, 
	\end{equation}  
...
\subsection{Analysis when Bob self-tests\label{subsec:SDP-when-Bob}}

\Anote{JAMIE: Feel free to replace this. The blue part is the old asymptotic version. After, I have added the idea for the full version. One step, still needs some work.}
...
% \Anote{I removed the lemma from here}
...
% \subsection{SDP when Alice self-tests}
% % \begin{prop}
